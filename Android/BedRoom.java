...
public class BedRoom extends ComponentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 1;i <= 7;i++) {
            int imgId = getResources().getIdentifier("bedRoomImage" + i, "id", getPackageName());
    ...
    public void setImageContent() {
        ...
        for(ImageView i:imageViews) {
            stref = FirebaseStorage.getInstance().getReference("bedroom" + j + ".jpg");
            stref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                @Override
                public void onSuccess(Uri uri) {
                    Glide.with(BedRoom.this).load(uri.toString()).into(i);
                }
            });
            j++;
        }
